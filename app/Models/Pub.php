...
class Pub extends Model
{
    ...
    {
        return $this->hasOne(PubsConfig::class);
    }


    public function coupon()
    {
        return $this->hasMany(Coupon::class,'pub_id');
    }
    public function notification_setup()
    {
        return $this->hasMany(RestaurantNotificationSetting::class,'pub_id');
    }
    public function coupon_valid()
    {
        return $this->hasMany(Coupon::class,'pub_id')->where('status',1)->whereDate('expire_date', '>=', date('Y-m-d'))->whereDate('start_date', '<=', date('Y-m-d'));
    }

    public function restaurant_sub()
    {
        return $this->hasOne(PubsSubscription::class)->where('status',1)->latestOfMany();
    }

    public function restaurant_subs()
    {
        return $this->hasMany(PubsSubscription::class,'pub_id');
    }
    ...
    {
        return $this->hasOne(PubsSubscription::class)->latestOfMany();
    }
    ...
    {
        return $this->hasMany(PubsSchedule::class)->orderBy('opening_time');
    }
    ...
    {
        $query->selectRaw('*, IF(((select count(*) from `pubs_schedules` where `pubs`.`id` = `pubs_schedules`.`pub_id` and `pubs_schedules`.`day` = '.now()->dayOfWeek.' and `pubs_schedules`.`opening_time` < "'.now()->format('H:i:s').'" and `pubs_schedules`.`closing_time` >"'.now()->format('H:i:s').'") > 0), true, false) as is_open,ST_Distance_Sphere(point(longitude, latitude),point('.$longitude.', '.$latitude.')) as distance');
    }
    ...
    public function scopeCuisine($query, $cuisine_id)
    {
        if($cuisine_id != 'all' && !is_array($cuisine_id)){
            return $query->whereHas('cuisine', function ($query) use ($cuisine_id){
                $query->where('cuisine_pub.cuisine_id', $cuisine_id);
            });
        }
            return $query;
    }
    public function scopeMultiCuisine($query, $cuisine_id)
    {
        ...
        if(is_array($cuisine_id) && count($cuisine_id)>0){
            return $query->whereHas('cuisine', function ($query) use ($cuisine_id){
                $query->whereIn('cuisine_pub.cuisine_id', $cuisine_id);
            });
        }
            return $query;
    }
    ...
    {
        return $this->hasMany(PubsSchedule::class)->orderBy('opening_time')->where('day',now()->dayOfWeek);
    }
    ...
    protected static function booted()
    {
        ...
        static::retrieved(function () {
            ...
            if($check_daily_subscription_validity_check && $check_daily_subscription_validity_check?->value != $current_date){
                Pub::whereHas('restaurant_sub',function ($query)use($current_date){
                ...
                PubsSubscription::where('status',1)->whereDate('expiry_date', '<=', $current_date)->update([
                    'status' => 0
                ]);
